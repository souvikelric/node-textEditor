...
const colouredMessage = (color, message) => { };
...
screen.key(["up", "down", "left", "right"], (ch, key) => {
    ...
    if (!isInsertMode && !commandMode) {
        if (key.name === "left" && cursorX > 0)
            cursorX--;
        if (key.name === "right" && cursorX < (((_a = buffer[cursorY]) === null || _a === void 0 ? void 0 : _a.length) || 0))
            cursorX++;
        if (key.name === "down" && cursorY < buffer.length - 1)
            cursorY++;
...
function centerText(lines) {
    const terminalWidth = Number(screen.width.valueOf());
    const terminalHeight = Number(screen.height.valueOf());
