...
screen.key(["h", "j", "k", "l"], (ch, key) => {
    var _a;
    if (key.name === "h" && cursorX > 0)
        cursorX--; // left
    if (key.name === "l" && cursorX < (((_a = buffer[cursorY]) === null || _a === void 0 ? void 0 : _a.length) || 0) - 1)
        cursorX++; // right
    if (key.name === "j" && cursorY < buffer.length - 1)
        cursorY++; // down
    if (key.name === "k" && cursorY > 0)
        cursorY--; // up
    render();
});
